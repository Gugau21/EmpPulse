...
-- ─────────────────────────────────────────────────────────────────────────────
-- Seed data (test fixtures for status filtering)
--
-- A handful of departments and employees so the Employees page has people to
-- filter. There is no status column in the schema — the UI assigns each person a
-- mock status from their id (see frontend mockEmployeeStatus), cycling
-- Working / Sick / Vacation / Personal — so inserting employees in a known order
-- gives a predictable spread across all four tags.
--
-- pass_hash is a throwaway placeholder: these users exist only to populate the
-- list and are not expected to sign in. The owner account is seeded separately
...
INSERT INTO app_user (name, surname, email, pass_hash) VALUES
  ('Alice',  'Johnson',  'alice.johnson@example.com',  'seed-no-login'),
  ('Bohdan', 'Koval',    'bohdan.koval@example.com',   'seed-no-login'),
  ('Chloe',  'Martin',   'chloe.martin@example.com',   'seed-no-login'),
  ('Dmytro', 'Shevchenko','dmytro.shevchenko@example.com', 'seed-no-login'),
  ('Emma',   'Nowak',    'emma.nowak@example.com',     'seed-no-login'),
  ('Filip',  'Wójcik',   'filip.wojcik@example.com',   'seed-no-login'),
  ('Hanna',  'Bondarenko','hanna.bondarenko@example.com', 'seed-no-login'),
  ('Igor',   'Tkachenko','igor.tkachenko@example.com', 'seed-no-login'),
  ('Julia',  'Lewandowska','julia.lewandowska@example.com', 'seed-no-login');

INSERT INTO employee (id, department_id, vacation_balance)
SELECT u.id, d.id, 20
FROM app_user u
JOIN (VALUES
  ('alice.johnson@example.com',      'Engineering'),
  ('bohdan.koval@example.com',       'Engineering'),
  ('chloe.martin@example.com',       'Engineering'),
  ('dmytro.shevchenko@example.com',  'Marketing'),
  ('emma.nowak@example.com',         'Marketing'),
  ('filip.wojcik@example.com',       'Marketing'),
  ('hanna.bondarenko@example.com',   'Sales'),
  ('igor.tkachenko@example.com',     'Sales'),
  ('julia.lewandowska@example.com',  'Sales')
) AS seed(email, dept) ON seed.email = u.email
JOIN department d ON d.name = seed.dept;